...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    if (empty($_POST["id"])){
        ...
        $id = $_POST["id"];
        $sql = "select yearmonth, SUM(step) as totalstep, COUNT(yearmonth) as daynum from step_records where id = ? GROUP BY yearmonth";
        ...
        if(empty($stmt)){
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                mysqli_stmt_bind_result($stmt, $yearmonth_var, $totalstep_var, $daynum_var);

                while (mysqli_stmt_fetch($stmt)) {
                    $recent_results[$yearmonth_var] = doubleval($totalstep_var) / doubleval($daynum_var);
                }
